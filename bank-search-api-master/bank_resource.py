...
class BankResource:
    ...
    def search(bank_query: str, branch_query: str):
        ...
        if not branch_matches and not address_matches:
            raise ValueError(f"No matching branch found for query: bank='{bank_query}', branch='{branch_query}'")

        best_branch = branch_matches[0] if branch_matches else None
        best_address = address_matches[0] if address_matches else None

        print(best_address, best_branch)
        if best_branch and best_address:
